...
RUN apt-get update \
  ...
  && if python3 -m pip install --help 2>/dev/null | grep -q -- '--break-system-packages'; then \
       python3 -m pip install --no-cache-dir --break-system-packages --ignore-installed click==8.3.1 requests==2.32.3; \
       python3 -m pip install --no-cache-dir --break-system-packages "faster-whisper==1.1.1"; \
     else \
       python3 -m pip install --no-cache-dir --ignore-installed click==8.3.1 requests==2.32.3; \
