      ...
      files: [
        {
          ...
          thumbnail: thumbnail
            ? {
                ...
                telegramFileId: thumbnail.telegramFileId,
                telegramChatId: thumbnail.telegramChatId,
                mirrorProvider: thumbnail.mirrorProvider,
                mirrorUrl: thumbnail.mirrorUrl,
                mirrorMessageId: thumbnail.mirrorMessageId,
                mirrorWebhookId: thumbnail.mirrorWebhookId,
                mirrorTelegramFileId: thumbnail.mirrorTelegramFileId,
                mirrorTelegramChatId: thumbnail.mirrorTelegramChatId,
                mirrorPending: thumbnail.mirrorPending,
                mirrorError: thumbnail.mirrorError,
      ...
      const scheduleUploads = () => {
        ...
        while (active < concurrency && pending.length > 0) {
          ...
          (async () => {
            ...
            noteUploadProviderDone(result.primary.provider, part.size);
...
apiRouter.get("/archives/:id/preview", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (heifPreview) {
      const jpegPath = path.join(tempDir, `${fileIndex}_${sanitizeName(fileName)}.preview.jpg`);
